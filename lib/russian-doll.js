...
function instantiate (config) {
    ...
    search: function (criteria, cb) {
      ...
      if (!is.ok(criteria) || !is.type(criteria, "object")) {
        return cb("usage: (queries {object}, cb {function})");
      }

      var query = map(criteria);
      var url = baseUrl + "/_search" + query;
      ...
      request(optsSearch, function (e, r) {

        if (e) {
          return cb(e, null);
        } else if (r.hits.total === 0) {
          return cb(null, []);
        } else {
          return cb(null, r.hits.hits.map(function (m) {
